...
\graphicspath{{../outputs/}{../outputs/xgboost/}}

\title{Detection-Probability-Oriented Waveform Co-Optimization and XGBoost Surrogate Analysis for ISAC Power Allocation}
...
\begin{abstract}
This paper replaces the usual SNR-based sensing metric in an integrated sensing and communication (ISAC) power-allocation problem with detection probability, so that waveform shaping becomes a direct design variable rather than a side detail. We study a weighted rate--detection formulation over a time-varying OFDM channel and optimize both power and a normalized waveform profile. The experiments use a \texttt{UMi\_NLOS} scenario with false-alarm probability $P_{\mathrm{fa}}=10^{-3}$, which defines the detection regime for the reported $P_d$ values. In the Pareto study, Grey Wolf Optimizer (GWO) spans the tradeoff from a sensing-focused point with $P_d=0.892$ at 10.70 bps/Hz to a communication-focused point with 22.76 bps/Hz and $P_d=0.212$. We further export GWO-generated optimization data across four channel scenarios and train an XGBoost surrogate to predict the weighted objective from channel and system features. The surrogate reaches $R^2=0.993$ on a held-out split of the current dataset, and feature-importance plus Tree SHAP plots expose which channel features most influence the learned optimized performance. The result is a combined optimization-and-explanation workflow: GWO produces allocation decisions, while XGBoost summarizes and interprets the resulting performance landscape.
\keywords{ISAC \and detection probability \and waveform co-optimization \and XGBoost \and surrogate modeling \and SHAP}
\end{abstract}
...
\begin{itemize}
  ...
  \item an exported GWO dataset for surrogate modeling across multiple channel scenarios,
  \item an XGBoost regression surrogate with feature-importance and Tree SHAP interpretation,
...
The added machine-learning layer follows a different purpose from the optimizer comparison. Gradient-boosted trees, and XGBoost in particular, are widely used for tabular regression and feature-ranking tasks \cite{chen2016xgboost}. In this paper, XGBoost is used after GWO has generated optimized results. It acts as a surrogate and explanation model, not as a replacement optimizer. Tree SHAP contribution values are then used to summarize how individual input features push the learned objective prediction up or down \cite{lundberg2017shap}.
...
\begin{itemize}
  ...
  \item the XGBoost results are trained on the exported simulation dataset and should be read as surrogate-analysis evidence, not as an independent physical model.
\end{itemize}
...
\subsection{XGBoost surrogate-analysis protocol}
After generating optimization results, we export a supervised-learning dataset from GWO runs. Each row corresponds to one channel snapshot and contains scenario parameters, OFDM/link-budget settings, channel-gain statistics, per-subcarrier communication gains, per-subcarrier sensing gains, and the resulting GWO metrics. The current exported dataset contains 80 rows: four channel scenarios with 20 time snapshots each. The scenarios are \texttt{ITU\_PedB}, \texttt{UMa\_LOS}, \texttt{UMi\_NLOS}, and \texttt{WINNER\_A1}.

The XGBoost model is trained to predict the GWO weighted objective. To avoid leakage, direct outcome columns are excluded from the input matrix, including the objective itself, rate, sensing SNR, detection probability, runtime, optimization-history values, optimized power allocation, and optimized waveform values. The model therefore learns from channel and system descriptors rather than from the answer columns.

\begin{table}[t]
\centering
\caption{XGBoost surrogate setup used for the exported GWO dataset.}
\label{tab:xgboostsetup}
\small
\begin{tabular}{lc}
\toprule
Item & Value \\
...
Dataset rows & 80 \\
Input features & 102 \\
Target & GWO weighted objective \\
Train/test split & 60 / 20 rows \\
Test fraction & 0.25 \\
Boosting rounds & 300 \\
...
The trained model is saved as a JSON booster, while separate files store feature metadata, held-out predictions, and evaluation metrics. This makes the analysis reproducible and allows the plotting scripts to create predicted-versus-actual, feature-importance, and SHAP summary figures without re-running GWO.
...
\subsection{XGBoost surrogate and interpretability results}
The XGBoost surrogate is evaluated on a held-out split of the exported GWO dataset. Table~\ref{tab:xgboostresults} summarizes the prediction quality. The high $R^2$ value shows that, for the current dataset, the channel and system descriptors contain enough information for the model to reproduce the GWO weighted objective with small error. This does not mean XGBoost has replaced GWO; it means that the GWO-generated performance landscape is learnable from the exported features.

\begin{table}[t]
\centering
\caption{Held-out XGBoost surrogate accuracy for predicting the GWO weighted objective.}
\label{tab:xgboostresults}
\small
\begin{tabular}{lc}
\toprule
Metric & Value \\
\midrule
Mean absolute error (MAE) & 2.322 \\
Root mean square error (RMSE) & 3.445 \\
$R^2$ score & 0.993 \\
Target mean & 77.382 \\
Target standard deviation & 49.648 \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:xgbpred} compares predicted and actual held-out objective values. Most points lie close to the ideal diagonal, which supports using the trained model as a compact surrogate for analysis and visualization.

\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{xgboost_predicted_vs_actual.png}
\caption{Held-out XGBoost predictions compared with the corresponding GWO weighted objective values.}
\label{fig:xgbpred}
\end{figure}

Feature importance and SHAP analysis give the model an explanatory role. The gain-based importance plot in Figure~\ref{fig:xgbimportance} ranks features by their average contribution to reducing tree-split loss. It gives a global view of which channel or scenario descriptors the boosted trees use most strongly.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{xgboost_feature_importance.png}
\caption{XGBoost feature-importance ranking for the weighted-objective surrogate.}
\label{fig:xgbimportance}
\end{figure}

Figure~\ref{fig:xgbshap} provides a SHAP-style summary using XGBoost's built-in Tree SHAP contribution values. Unlike the gain plot, which only ranks global importance, SHAP values show whether high or low feature values tend to push the predicted optimized objective upward or downward. This helps connect the learned surrogate back to the physical channel descriptors used in the ISAC simulation.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{xgboost_shap_summary.png}
\caption{Tree SHAP summary for the XGBoost surrogate. Positive SHAP values increase the predicted weighted objective, while negative values reduce it.}
\label{fig:xgbshap}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{The GWO performance landscape is learnable.} XGBoost predicts the exported GWO weighted objective accurately on the held-out split, and its importance and SHAP plots make the learned dependence on channel features easier to inspect.
\end{enumerate}
...
\begin{itemize}
  ...
  \item The XGBoost surrogate is trained on 80 exported rows, so the reported accuracy is best read as a first proof of learnability rather than a final generalization claim.
  \item The SHAP summary explains the trained surrogate model, not the physical system directly; physical interpretation should therefore be tied back to the channel model and exported features.
\end{itemize}

These points should be stated clearly in the manuscript. Doing so does not weaken the paper; it makes it more credible. The natural future steps are Monte Carlo averaging, more detailed sensing physics, additional baselines that operate on the detector side rather than only on the allocation side, larger XGBoost training datasets, cross-validation, and response-surface plots generated from the trained surrogate.

\section{Conclusion}
This paper presents a focused ISAC case study for detection-probability-oriented power and waveform co-optimization. The main message is that detection probability and waveform shaping should be treated as first-class design variables if the sensing task is detection-oriented. Under the fixed-seed protocol used here, GWO has a slight edge on the weighted objective and PSO is a close and often faster alternative; power-only baselines such as water-filling remain strong on rate but weak on detection. The Pareto endpoints and the non-flat optimized waveform profiles support the practical value of detection-aware waveform co-optimization. The added XGBoost layer extends the study from optimization to explanation: it learns the exported GWO objective with high held-out accuracy and provides feature-importance and SHAP plots that help identify which channel and system descriptors drive optimized performance.
